...
add_acc(DeliSize, Bytes, [Tail | Header]) ->
    ...
    case Size =< 0 of
        ...
        false ->
            ...
            %% cut
            <<TailH:Size/binary, TailT/binary>> = Tail,
            %% new tail
            NewTail = <<TailT/binary, Bytes/binary>>,
...
make_msgs([]) -> [];
make_msgs([K, V | Rest]) -> [make_msg(K, V) | make_msgs(Rest)].
